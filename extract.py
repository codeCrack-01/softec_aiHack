...
# No longer explicitly importing HumanMessage, SystemMessage as dict format for conversation works
...
# Create an llm object using GPT open source model
# Ensure the model name is correct, "openai/gpt-oss-120b" seems unusual for Groq
llm = ChatGroq(
    groq_api_key=os.getenv("GROQ_API_KEY"), model="openai/gpt-oss-120b"
)  # Changed to a common Groq model
...
def process_emails(emails):
    ...
    for email in emails:
        ...
        # Format the EXTRACTION_PROMPT with the actual email body
        formatted_extraction_prompt = EXTRACTION_PROMPT.format(email_text=email["body"])

        # Call the LLM with the formatted prompt as system message and a simple user query
        ...
        try:
            ...
            if cleaned_response.startswith("```json") and cleaned_response.endswith(
                ...
                # Handle generic code block if not specifically json
            ...
            processed_emails.append(
                {
                    ...
                    "llm_raw_response": llm_response_content,  # Keep raw response for debugging
                }
            )
...
def rank_emails(student_profile, processed_emails):
    ...
    for email in processed_emails:
        # Ensure email is a dictionary and has 'is_opportunity' key
        # Also check if it's an error dict from parsing
        if (
        ...
        if required_degree in (None, ""):
            score += 15  # no restriction, don't penalize
        elif (
            student_profile.get("degree", "").lower() == (required_degree or "").lower()
        ):
            score += 15

        # --- CGPA match (High weight) ---
        ...
        # --- Semester range match (Medium weight) ---
        ...
        if deadline_str is not None:
            try:
                ...
                pass  # malformed date, skip urgency scoring
        ...
        ranked_emails.append(
            [email, score]
        )  # Append the processed email dict and its score
...
def explain_emails(student_profile, ranked_emails):
    ...
    for opportunity_with_score in ranked_emails:
        ...
        # Format the EXPLANATION_PROMPT with both student_profile and the current opportunity
        # Use json.dumps for safe conversion of dicts to JSON strings for the prompt
        ...
        # The user_input for EXPLANATION_PROMPT can be an empty string if the prompt is self-contained.
...
def get_llm_response(prompt, user_input):
    # Design a conversation of the user and the llm
    conversation = [
        {"role": "system", "content": prompt},
        {"role": "user", "content": user_input},
    ]

    # Make a call to the llm and get the response
